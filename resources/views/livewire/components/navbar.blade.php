<header
    class="sticky top-0 z-50 flex flex-wrap w-full py-6 text-white shadow-md bg-gradient-to-r from-gray-900 via-gray-800 to-gray-700 md:justify-start md:flex-nowrap md:py-0 dark:bg-gray-800">
    <nav class="max-w-[85rem] w-full mx-auto px-4 md:px-6 lg:px-8" aria-label="Global">
        <div class="relative md:flex md:items-center md:justify-between">
            <div class="flex items-center justify-between">
                ...
                <div class="md:hidden">
                    <button type="button"
                        class="flex items-center justify-center text-gray-300 transition hover:text-white focus:outline-none hs-collapse-toggle w-9 h-9 disabled:opacity-50 disabled:pointer-events-none"
                        data-hs-collapse="#navbar-collapse-with-animation"
                        aria-controls="navbar-collapse-with-animation" aria-label="Toggle navigation">
                        <svg class="flex-shrink-0 w-6 h-6 hs-collapse-open:hidden" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <svg class="flex-shrink-0 hidden w-6 h-6 hs-collapse-open:block"
            ...
            <div id="navbar-collapse-with-animation"
                ...
                <div
                    ...
                    <div
                        ...
                        <a wire:navigate
                            class="py-3 font-medium {{ request()->routeIs('home') ? 'text-white' : 'text-gray-400' }} transition duration-300 md:py-6 hover:text-white"
                            href="{{ route('home') }}" aria-current="page">Home</a>

                        <a wire:navigate
                            class="py-3 font-medium {{ request()->routeIs('categories') ? 'text-white' : 'text-gray-400' }} transition duration-300 md:py-6 hover:text-white"
                            href="{{ route('categories') }}">
                            Categories
                        </a>

                        <a wire:navigate
                            class="py-3 font-medium {{ request()->routeIs('products') ? 'text-white' : 'text-gray-400' }} transition duration-300 md:py-6 hover:text-white"
                            href="{{ route('products') }}">
                            Products
                        </a>

                        <a wire:navigate
                            class="flex items-center py-3 font-medium {{ request()->routeIs('cart') ? 'text-white' : 'text-gray-400' }} transition duration-300 hover:text-white"
                            href="{{ route('cart') }}">
                            ...
                            <span class="mr-1">Cart</span> <span
                                class="py-0.5 px-1.5 rounded-full text-xs font-medium bg-blue-50 border border-blue-200 text-black">{{ $totalItems }}</span>
                        </a>
                        @guest
                        ...
                        @auth
                            <div
                                ...
                                <button type="button"
                                    class="flex items-center w-full font-medium text-gray-400 hover:text-white dark:text-white dark:hover:text-gray-400">
                                    {{ auth()->user()->name }}
