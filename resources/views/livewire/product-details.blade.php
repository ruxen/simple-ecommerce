<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
    <section class="py-4 overflow-hidden bg-white font-poppins dark:bg-gray-800">
                    ...
                    <div class="lg:pl-20">
                        <div class="mb-8 [&>ul]:list-disc [&>ul]:ml-4">
                            <h2 class="max-w-xl mb-2 text-2xl font-bold dark:text-gray-400 md:text-4xl">
                                {{ $product->name }}</h2>
                            <p class="font-semibold text-gray-600 text-md dark:text-gray-500">SKU:
                                {{ $product->sku }}</p>
                            <p class="mb-4 font-semibold text-gray-600 text-md dark:text-gray-500">Stock:
                                {{ $product->stock > 0 ? $product->stock . ' available' : 'Out of stock' }}</p>
                        ...
                        <div class="flex flex-wrap items-center gap-4">
                            <button
                                wire:click.prevent='@if ($product->stock > 0) addToCart({{ $product->id }}) @endif'
                                class="w-full p-4
                                    @if ($product->stock == 0) bg-gray-400 cursor-not-allowed
                                    @else
                                        bg-blue-500 hover:bg-blue-600 dark:bg-blue-500 dark:hover:bg-blue-700 @endif
                                    rounded-md lg:w-2/5 text-gray-50 dark:text-gray-200"
